...
const Offerings = () => {
  ...
  return (
    <section id="offerings" className="bg-white py-[100px] px-[5%] overflow-hidden">
              ...
              <button
                ...
                className={`flex items-center justify-between w-full min-w-[200px] lg:min-w-0 px-6 py-4 rounded-xl text-left transition-all duration-200 border ${
                  active === o.id 
                    ...
                    : 'bg-white border-[#E2E8F0] text-[#4A5568] hover:bg-blue-light hover:border-primary-blue/30'
                }`}
              >
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-10 pb-8 border-b border-navy-900/10">
                  <div className="max-w-xl">
                    <h3 className="font-serif text-3xl font-bold text-navy-900 mb-4">{data.title}</h3>
                    <p className="text-[15px] text-[#4A5568] leading-relaxed font-medium italic">"{data.definition}"</p>
                  </div>
                  <div className="bg-white p-6 rounded-2xl border border-primary-blue/10 shadow-sm shrink-0 w-full md:w-auto">
                    <div className="text-[10px] font-bold text-primary-blue tracking-wider uppercase mb-3">KEY BENEFITS</div>
                    <ul className="space-y-2">
                      {data.benefits.map((b, i) => (
                        <li key={i} className="flex gap-2 text-[12px] font-bold text-navy-900">
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-10">
                  {/* COVERAGE */}
                  <div>
                    <h4 className="text-[11px] font-bold text-primary-blue tracking-wider uppercase mb-6 flex items-center gap-2">
                    ...
                    <div className="space-y-4">
                      {data.coverage.map((c, i) => (
                        <div key={i} className="bg-white p-4 rounded-xl border border-navy-900/5 hover:border-primary-blue/20 transition-all">
                          <div className="text-[13px] font-bold text-navy-900 mb-1">{c.item}</div>
                          <p className="text-[11px] text-[#718096] leading-snug">{c.detail}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {data.pricing.map((p, i) => (
                        <div key={i} className="bg-navy-900 text-white p-4 rounded-xl flex justify-between items-center group">
                          <div>
                            <div className="text-[12px] font-bold">{p.tier}</div>
                            {p.bestFor && <div className="text-[10px] text-white/40">{p.bestFor}</div>}
                          </div>
                          <div className="text-right">
                            <div className="text-[18px] font-accent text-primary-blue">{p.premium}</div>
                    ...
                    {data.exclusions && (
                      <div className="mt-8 bg-red-50 p-4 rounded-xl border border-red-100">
                        <h5 className="text-[10px] font-bold text-red-600 tracking-wider uppercase mb-2">NOT COVERED (EXCLUSIONS)</h5>
                ...
                <div className="bg-primary-blue text-white rounded-[24px] p-8 relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <h4 className="text-[10px] font-bold text-white/50 tracking-wider uppercase mb-4 flex items-center gap-2">
                      <HelpCircle size={14} /> REAL-WORLD USE CASE
                    </h4>
                    <div className="text-xl font-serif font-bold mb-3">{data.useCase.title}</div>
                    <p className="text-[15px] leading-relaxed text-white/80 italic font-medium">
                      "{data.useCase.desc}"
                    </p>
                  </div>
                </div>
